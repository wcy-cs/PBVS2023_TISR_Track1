...
def calc_metrics(out, target, loss_fn_alex):
	...
	return np.array([psnr], dtype=float) #np.array([psnr, SSIM, LPIPS], dtype=float)
...
# 			if 'SR' in args.dataroot[-6:]:
# 				gt = cv2.imread(args.dataroot + '/test/' + name[:5] + '/target_x4_rgb/' + name[-5:] + '.png')[..., ::-1]
# 				# gt = cv2.imread(args.dataroot + '/test/' + name[:5] + '/target_x4_rgb/00006.png')[..., ::-1]
# 			else:
