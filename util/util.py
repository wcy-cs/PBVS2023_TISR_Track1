...
def torch_psnr(img1, img2, border=0, data_range=255.):
    if border != 0:
...
def calc_psnr1(sr, hr, range=255.):
    # shave = 2
    try:
        hr = rg(sr, hr)
    except:
...
def rg(img1, img2):
    ...
    # cv2.imwrite('/Volumes/Works/challengedataset/train/matches.jpg', img3)

    ref_matched_kpts = np.float32([kp1[m[0].queryIdx].pt for m in good_matches])
    sensed_matched_kpts = np.float32([kp2[m[0].trainIdx].pt for m in good_matches])

    # Compute homography
    H, status = cv2.findHomography(sensed_matched_kpts, ref_matched_kpts, cv2.RHO, 5.0)
